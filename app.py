...
if not st.session_state["giris_yapildi"]:
    ...
    with col_l2:
        ...
        # Checkbox kutusunu formun dışına, tam üstüne alarak CSS baskısından kurtarıyoruz
    ...
    if st.session_state["rol"] == "sube" and menu_secim == "Personel Giriş / Çıkış":
        ...
        with col_sol_form:
            ...
            # GÜVENLİK KİLİDİ: Şantiyeler sadece beklemede olan (onaylanmamış) kayıtları güncelleyebilir!
            ...
            if islem_modu == "Var Olan Personeli Güncelle / Çıkış Yap" and not df_guncellenebilir_havuz.empty:
                ...
                if secilen_g_p:
                    g_sira_no = int(secilen_g_p.split(" | ").replace("Sıra No: ", "").strip())
                    p_satir = df_guncellenebilir_havuz[df_guncellenebilir_havuz["Sıra No"] == g_sira_no].iloc
            ...
            if not df_goster.empty:
                ...
                if st.button("❌ SEÇİLİ PERSONELİ LİSTEDEN KALDIR", use_container_width=True):
                    s_sira = int(secilen_sil_p_sube.split(" | ").replace("Sıra No: ", "").strip())
                    p_durumu_kontrol = df_canli[df_canli["Sıra No"] == s_sira]["Giriş/Çıkış Durumu"].values
        ...
        with tab1:
            ...
            csv_m = kurumsal_rapor_uret(df_m_goster)
            st.download_button(label="📥 SEÇİLİ RESMİ LİSTEYİ DIŞARI RAPORLA", data=csv_m, file_name="merkez_personel_listesi.csv", mime="text/csv", use_container_width=True)
            ...
            if not df_canli.empty:
                ...
                if st.button("❌ SEÇİLİ PERSONELİ VERİTABANINDAN TAMAMEN UÇUR", use_container_width=True):
                    m_s_sira = int(secilen_m_sil_p.split(" | ").replace("Sıra No: ", "").strip())
        ...
        with tab2:
            ...
            csv_p = kurumsal_rapor_uret(df_p_m_goster)
            st.download_button(label="📥 TOPLU PUANTAJ LİSTESİNİ RAPOR YAP", data=csv_p, file_name="merkez_toplu_puantaj.csv", mime="text/csv", use_container_width=True)
            ...
            if not df_puantaj_canli.empty:
                ...
                if st.button("❌ SEÇİLİ PUANTAJI VERİTABANINDAN KALICI OLARAK SİL", use_container_width=True):
                    m_sil_tarih = secilen_m_sil.split(" | ").strip()
